...
#define BLIP_ID 5 // set this to a unique ID to disambiguate multiple nodes
...
struct {
  ...
  byte postein; //Einwurfdeckel geöffnet
  byte postaus;  //Entnahmetür geöffnet
} payload;

volatile int seconds  = 60;          // send status every 60 seconds
...
void setup() {
  ...
  seconds = 2; // first send quick after reset
...
void loop() {
  ...
  if (irqFlag1==1) {                  // Flag was set by IRQ1 routine
      ...
      payload.postein = counter1;
       counter2 = 0;
      payload.postaus = counter2;
      sendPayload();
