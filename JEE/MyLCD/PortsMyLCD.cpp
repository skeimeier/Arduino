...
void LiquidCrystalI2C::config() {
  ...
  device.send();
  device.write(MCP_IODIR0);
  device.write(0x00);
  device.write(0x00);
  device.stop();
  device.send();
  
  device.write(MCP_GP0);
  device.write(0x00);
  device.write(0x00);
  device.stop();
...
void LiquidCrystalI2C::send(byte value, byte mode) {
  _control = mode;
  if (_control != 0)
    _control = MCP_REGSEL;   // command or data
  write8bits(value);
}
...
void LiquidCrystalI2C::write8bits(byte value) {
  _control |= MCP_BACKLIGHT | MCP_ENABLE;
  device.send();
  device.write(MCP_GP0);
  device.write(_control);
  device.write(value);
  device.write(_control ^ MCP_ENABLE);
  device.write(value);
  device.stop();
}
