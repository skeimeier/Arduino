...
volatile byte next, fill, start, ende, buf[256];
static word last;
   ...
   /*if(b == 255){
      if(start == 0){
          ...
          Serial.print("start=");Serial.println(start);
            ...
            ende = next;
            Serial.println(); Serial.print("ende="),Serial.println(ende);
           word n = start;
           byte nullcount = 0;
           byte bitno = 0;
           byte sync = 0;
            while( n <= ende){
               bitno++;
               sync++;
               word c = buf[n] + buf[n+1];
               if(c > 210){ 
                 if(nullcount < 12)
                    Serial.println(" 1 nullcount error");
                 if(bitno == 13){ 
                     Serial.println(" 1 header");
                     sync = 0;
                 }else if((bitno > 14) &&  (sync == 5)){
               ...
               if((bitno > 14) &&  (sync == 5)){
                ...
                n = n+2;
                // if((start-n) % 8 == 0 ) Serial.println();
               
            }
            start = ende = next = fill = 0;
        }
