...
static uint8_t myIP[] = {192,168,115,132};
...
static void collectPayload () {
  ...
  byte len = rf12_len; // this many values will be needed
  for (char i = 0; i <  len; ++i) {// include -1, i.e. the length byte
    sprintf(buf," %d", rf12_data[i]);
    collectStr(buf);
  }
}
...
static void forwardToUDP () {
  ...
  collPos = 0;
  collectStr("JeeUdp_01 ");
  word mhz = config.band == 4 ? 433 : config.band == 8 ? 868 : 915;
  sprintf(buf, "RF12@%d Group:%d ", mhz, config.group);
  collectStr(buf);
  collectStr("OK ");
  sprintf(buf, "ID:%d", rf12_hdr);
  collectStr(buf);
  collectPayload();
