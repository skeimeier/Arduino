...
#define BLIP_GRP  18   // wireless net group to use for sending blips
#define BLIP_ID   13   // set this to a unique ID to disambiguate multiple nodes
#define SEND_MODE 3   // set to 3 if fuses are e=06/h=DE/l=CE, else set to 2

#define ALARM_PIN 10 //pin 10 (JEE DIO1) low löst alarm aus

long  zehntelsekundenTimer;  
...
struct {
  ...
  byte alarm;     // pin 10 (JEE DIO1) low löst alarm aus
} payload;
...
void setup() {
  ...
  digitalWrite(ALARM_PIN,HIGH); // Pullup aktivieren
  payload.alarm = 0;
  sendPayload();
...
void loop() {
  ...
  if(zehntelsekundenTimer-- > 0){
    ...
    rf12_sleep(RF12_SLEEP);
    zehntelsekundenTimer = 600;  // eine Minute/  Timer reload
  }
/*
  byte vcc = payload.vcc1 = vccRead();
