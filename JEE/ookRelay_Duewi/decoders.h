...
/// OOK decoder for Düwi-Bewegungsmelder devices.
class DuewiBWMDecoder : public DecodeOOK {
public:
    DuewiBWMDecoder () {}
    
    virtual char decode (word width) {
        if (180 <= width && width < 520) {

            byte w = width >= 350;
            switch (state) {
                case UNKNOWN:  //sync abwarten 
                //case OK:
                    state = T2;
                    break;
                case T2:
