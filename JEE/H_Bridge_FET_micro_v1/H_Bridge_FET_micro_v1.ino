...
#define MAX_DAUER 15 // maximale Ventieloffen-Dauer in MINUTEN

#define BLIP_GRP  18   // wireless net group to use for sending blips
#define BLIP_NODE 13  // wireless node ID to use for sending blips
...
// give it a name:
int a_side = 10;  // PORT 1 DIO
int b_side = 9; // PORT 1 AIO
int led_a = 7;  //Testled
int led_b = 8;  //Testled
int tast = 3; //direktschalter
...
void setup() {   
 ...
 pinMode(tast, INPUT);
 digitalWrite(tast,HIGH); // Pullup aktivieren
...
void mot_fwd() {
  ...
  digitalWrite(b_side, LOW);   // turn the LED on (HIGH is the voltage level) 
   Sleepy::loseSomeTime(1000); // 800 ms warten
