...
#define PC_INT_PIN    6   // JEE PORT3 DIO = AVR PD6
#define boardLed 9
...
void initStromZaehler(){
 ...
 // PCintPort::attachInterrupt(PIN1, &PCintFunc, FALLING);  // add more attachInterrupt code as required
}

void doStromCount(){
  digitalWrite(boardLed,digitalRead(PC_INT_PIN));
  if(interrupt_count < 38000L){
    if (count != 0) {
      if(interrupt_count > 1){
         schnitt = mittelwert_filter(gap);
      }
      if( ( millis()-lastminute) > 1000L){ // alle 1 sekunden
            lastminute = millis();
      ...
      if( (interrupt_count > 1) && (gap > (5*schnitt)) ){
        ...
        watt_last_round = 37500000L/rundenzeit;
             ...
             lcd.print(" ");
             lcd.print(watt_last_round); 
             lcd.print(" W");
#if SERIAL
        Serial.print(F(">>> Runden "));
...
static void homePage(BufferFiller& buf) {
    buf.emit_p(PSTR("$F\r\n"
        ...
        "<h3>Zaehler:</h3>"
        "<P>Verbrauch aktuell: $L Watt</P>"
        "<P>Runden: $L Ticks: $L Rundenzeit: $L ms </P>"
        "<P>Verbrauch letzte Runde: $L Watt</P>"
        "<P>Verbrauch Total: $L.$L kWh</P>"), okHeader,10, watt_mittel, runden, last_int_count, rundenzeit, watt_last_round, runden/96,(runden*1000)%96);
    long t = millis() / 1000;
    word h = t / 3600;
    byte m = (t / 60) % 60;
    byte s = t % 60;
