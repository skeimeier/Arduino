...
int tempTemp=153; // Tempory fitictous value for temprature
...
void get_and_do_command()
{
	...
	if (Serial.available())
	{
		c = Serial.read();
                if(c == '\r') c = '\n';
                 if(c == '*')c = '\n';   // test so you can enter commands from the Arduino's monitor
	...
	if (serial_count && c == '\n')
	{
		...
		init_process_string();
             ...
             //   delay(50);  // just to allow the LCD to display remove on final code
...
float feedrate = DEFAULT_XY_FEEDRATE;
...
void process_string(char instruction[], int size)
{
	...
	if (instruction[0] == '/')	
       ...
       lcd.write(instruction[i]);
        }
        lcd.print(" <- ");      // Just seprate it from any other junk on the line
...
// #endif
			...
			case 17: //set circle plane to X Y
				// we currently do'y support anything else so just say it's ok
				break;
			...
			// Drilling canned cycles
                        case 70: enable_steppers();  // for setting motor quesent current
	...
	if (gc.seen & GCODE_M)
	{
		switch (gc.M)
		{
			...
			case 0:
				 ...
				 */
                                 case 17:
				 //todo: // enable stepping motors
				 break;

                                 case 18:
				 //todo:  // disable stepping motors
				 break;
			...
			case 108:
				if (gc.seen & GCODE_S)
					analogWrite(SPINDLE_PIN, gc.S);  // control spindle motor speed
				break;
